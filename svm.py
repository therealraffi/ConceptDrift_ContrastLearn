...
meta = pd.read_csv("data/dataset/apk_metadata.csv", parse_dates=["datetime"]).set_index("apk_id")
rg = RelationGraph(
    rel_path="APIGraph/src/res/relations.txt",
    ent_path="APIGraph/src/res/entities.txt",
...
sample_path = next(Path("data/subgraphs").glob("*.gpickle"))
sample_data = to_heterodata(pickle.load(open(sample_path, "rb")), rg)
metadata = sample_data.metadata()

dataset = ContrastiveGraphDataset(
    graph_dir="data/subgraphs",
    meta=meta,
    rg=rg,
    metadata=metadata,
    num_pairs=5000,
    sample_pairs=5000,
    pair_file="data/dataset/pairs.pkl",
    save_pairs=False
)
